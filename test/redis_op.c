...
int rop_range_list(redisContext *conn, char *key, int from_pos, int count, RVALUES values, int *get_num)
{
    ...
    reply = redisCommand(conn, "LRANGE %s %d %d", key, from_pos, count);//0--------10
//    rop_test_reply_type(reply);
	if (reply->type != REDIS_REPLY_ARRAY) {
...
int rop_string_set(redisContext *conn,char *key,char *value)
{
    int ret = 0;
	redisReply *reply = NULL;

    if(conn==NULL||key==NULL||value==NULL)
    {
        LOG(REDIS_LOG_MODULE,REDIS_LOG_PROC,"[-][GMS_REDIS] rop_string_set param error");
        goto END;
    }
